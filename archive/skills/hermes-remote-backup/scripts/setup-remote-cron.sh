...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --user)      HOST="${2}@${HOST}"; shift 2 ;;
    --time)      CRON_HOUR="${2%%:*}"; CRON_MIN="${2##*:}"; shift 2
      if ! echo "$CRON_HOUR" | grep -qxE '[0-9]{1,2}' || [ "${CRON_HOUR#0}" -gt 23 ] 2>/dev/null; then
        echo "ERROR: Invalid --time hour: $CRON_HOUR (must be 0-23)" >&2; exit 1
      fi
      if ! echo "$CRON_MIN" | grep -qxE '[0-9]{1,2}' || [ "${CRON_MIN#0}" -gt 59 ] 2>/dev/null; then
...
CRON_CMDS="#!/bin/bash
# Auto-generated by hermes-remote-backup setup-remote-cron.sh
