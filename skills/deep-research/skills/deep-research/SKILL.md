...
deep-research runs in one of two modes. **Unattended is the default for any model/agent auto-invocation**: another agent or skill spawning this skill, headless `-p` runs, schedulers, or non-TTY sessions. Interactive mode applies only when a human types the slash command in an attended session.
...
| `interactive` | User typed `/deep-research …` in an attended TUI (not headless), and did **not** pass `--unattended` / `--ephemeral` / smoke flags | Ask **only** when ambiguity is **blocking** (cannot research without a choice) — at most one focused question | Skill default (vault auto when available) unless user flags |
...
|----------|--------------------|
| Output | **stdout / `--ephemeral`** for automated and agent-spawned runs; vault only if the parent explicitly requested persistence via `--vault` / `--save` |
...
Do NOT use for:
- Quick factual lookups or single-fact freshness questions (latest version, one changelog line) — **Prefer /grok-search** (or grok-search MCP) for a single latest-version / one-URL fact. If deep-research is invoked anyway, run the full orchestrator (no skip).
...
- **Claim caps**: each per-question researcher returns at most 6 atomic claims (claim + evidence + source). Across all questions the candidate claim pool is capped at 24. Over-cap claims are dropped before synthesis; the Coverage and uncertainty section notes the count and which questions exceeded their cap.
...
- **Deterministic scratch pipeline and fallback checkpointing**:
  When no caller/runner supplies artifact paths, resolve a safe run-scoped temporary scratch directory outside the vault: `${TMPDIR:-/tmp}/deep-research/<unique-run-id>/` (created via `mktemp -d` when terminal execution is available).
  Under this scratch directory, manage the exact run artifacts:
  - `fallback-input.json`: structured retained evidence and candidate ledger rows
  - `fallback.md`: deterministic valid Partial fallback report generated by `scripts/build-fallback-report.py`
  - `candidate.md`: synthesized report draft
  - `lint.json`: candidate linter output
  - `selection.json`: candidate selection audit and decisions
  - `final-report.md`: selected output report before Phase 5 routing

  Run commands using the plugin-root script paths:
  ```bash
  python3 scripts/build-fallback-report.py <scratch>/fallback-input.json --output <scratch>/fallback.md --artifact-root <scratch>
  python3 scripts/select-report-candidate.py --candidate <scratch>/candidate.md --fallback <scratch>/fallback.md --output <scratch>/final-report.md --lint-json <scratch>/lint.json --selection-json <scratch>/selection.json --artifact-root <scratch>
  ```
  If terminal or filesystem execution is unavailable, persist the structured Partial fallback in memory context and evaluate the candidate/repair/fallback selection ladder without claiming tool execution.
  **Safety invariant**: never write scratch artifacts into the vault; temporary scratch clean-up is best-effort after final report delivery, not before selection.

- **Pre-build deterministic fallback**: before composing the report, construct the retained-claim and source-ledger JSON at `fallback-input.json` (or in context) and run `scripts/build-fallback-report.py` to build the linter-clean `fallback.md` checkpoint before normal synthesis. The builder uses retained structured evidence, not malformed final prose. If normal synthesis is empty, malformed, omits required content, or fails the Mermaid syntax check, candidate selection emits the **structurally valid fallback** from `references/report-presentation-template.md`: `**Status: Partial**`, **Status header, H1 title**, `## 1. Findings`, **all four audit headings**, the preserved ledger, and an **explicit evidence-gap Coverage entry**. The builder and selector never change or invent evidence, claims, URLs, source roles, hashes, or Verified status; fallback linter failure is a hard report-contract failure. **Never return an empty report** or an isolated Findings list.
...
Run `skills/deep-research/scripts/lint-report.py` against every generated report before capture review. The linter strips a leading closed YAML frontmatter document and ignores H2s after `## Coverage and uncertainty` (vault `## Related Notes`). It validates heading order, ledger/citation mapping, source disclosure, local-record durability, cutoff, status/Coverage consistency, and model-narrated numeric tool counts. Candidate selection follows: candidate lint → repair → re-lint → fallback selection. If candidate lint reports identity or ledger-label errors, run `scripts/repair-report-structure.py` (structure-only) and re-run `lint-report.py`. If candidate remains invalid, `scripts/select-report-candidate.py` selects the prebuilt fallback checkpoint. The repairer may insert or move the H1 and prefix English role / `local-record:` tokens. Never change Status, claims, URLs, or Coverage to make lint pass. Fallback is always Partial with a synthesis-format Evidence gap. Fallback linter failure is a hard report-contract failure. Record leftover errors beside the raw report.

**Report presentation contract.** deep-research defaults to the bundled template at `references/report-presentation-template.md`. The exact generated-report form is defined in the Phase 3 contract above and validated by `scripts/lint-report.py`: status → localized H1 identity/cutoff/scope → compact navigation → sequential plain-ASCII numbered narrative H2s with plain ASCII ordinal prefixes → the four literal audit headings. Keep TL;DR decision-only, narrative facts single-home, timeline tables canonical, Mermaid visual-only, and Coverage limited to classifications/gaps. `Partial` remains based on evidence gaps, not presentation polish.
...
- **scripts/build-fallback-report.py**: Deterministic pre-synthesis fallback report builder
- **scripts/select-report-candidate.py**: Report candidate selector (normal, repaired, prebuilt fallback)
