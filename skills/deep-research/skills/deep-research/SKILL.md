...
- **Deterministic scratch pipeline and fallback checkpointing**:
  When no caller/runner supplies artifact paths, resolve the active production plugin root and execute a vault-safe scratch pipeline.
  **Plugin root resolution**: The agent must resolve the active production plugin root from the loaded `skills/deep-research/SKILL.md` path (two directories above that nested skill directory) and assign its absolute path to `DEEP_RESEARCH_PLUGIN_ROOT`; never assume a host environment variable exists.

  **Executable scratch setup and vault containment guard**:
  ```bash
...
vault_arg = sys.argv[2] if len(sys.argv) > 2 else ""
if not scratch_arg:
    raise SystemExit(1)
scratch = Path(scratch_arg).expanduser().resolve()
if not vault_arg or vault_arg == "NO_VAULT_CONFIGURED":
    print("outside")
    raise SystemExit(0)
try:
