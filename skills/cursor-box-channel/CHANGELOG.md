...
### Added
- Document optional grok.com Chat/Bot CDP attach: route-by-target (`newbie` vs `wiki-research`), dual endpoints, attach-only (never launches Chrome, navigates tabs, or closes browser), and message-id-only wake text. Kept default-off; Grok Bot routine webhook remains the Bearer-primary wake; HMAC extra remains default-off.
