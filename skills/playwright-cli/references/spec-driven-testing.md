...
Goal: produce a spec file (e.g. `specs/.plan.md`) that enumerates the scenarios to test. **Always** write the spec to a file.
...
Save under `specs/.plan.md`. Use this structure:

```markdown
# <Feature> Test Plan

## Application Overview
<brief description of the app under test>
...
#### 1.1. <scenario-kebab-case>
**File:** `tests/<group>/<scenario-kebab-case>.spec.ts`
**Steps:**
1. <user-level step>
...
- Prefix each numbered step with a `// N. ` comment before its actions.
...
```bash
PLAYWRIGHT_HTML_OPEN=never npx playwright test tests/<file>.spec.ts:<line> --debug=cli
