...
const BRAINSTORMING_VERSION = readBrainstormingVersion();
...
// External branding traffic is disabled by default in this port. It is enabled
// only when the operator explicitly opts in and no standard disable flag wins.
const BRAINSTORM_TELEMETRY_ENABLED =
  isTruthyEnv(process.env.BRAINSTORM_ENABLE_TELEMETRY) &&
...
function readBrainstormingVersion() {
...
function brandMarkup() {
  ...
  const text = 'Brainstorming Companion v' + version;
  const logo = BRAINSTORM_TELEMETRY_ENABLED
    ? '<img class="brand-logo" src="' + SUPERPOWERS_BRAND_IMAGE_URL + '?v=' + encodeURIComponent(BRAINSTORMING_VERSION) + '" alt="Prime Radiant" referrerpolicy="no-referrer" decoding="async">'
    : '';

  return '<div class="brand"><a href="https://github.com/karlorz/agent-skills/tree/main/skills/brainstorming">' + logo + '<span class="brand-copy">' + text + '</span></a></div>';
}
