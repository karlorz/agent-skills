#!/usr/bin/env python3
"""
test-report-presentation-template.py — static contract test for the future
D-owned report-presentation template.
...
and the interactive-only `--reuse-s-template` reuse contract with bundled
fallback and `--unattended` disablement.  No live model, network, Docker, or
vault access is required — this is a pure text-anchor check on one Markdown
file.

Run:
    python3 skills/deep-research-dev/scripts/test-report-presentation-template.py
...
def _read(path: Path) -> str:
    if not path.is_file():
        print(
            f"template FAIL: future template file not found: {path.resolve()}",
            file=sys.stderr,
        )
        sys.exit(1)
...
# ── Required anchors (approved presentation/source-ledger plan) ──────────────
...
# Interactive-only --reuse-s-template: structure-only reuse, safe bundled
# fallback, no S facts/sources/conclusions carried over, disabled under
# --unattended.
required_reuse_rules = (
    "--reuse-s-template",
    "structure-only",
    "bundled template",
...
def _check_ledger_fields(text: str, failures: list[str]) -> None:
    """The ledger header row must sit inside the `## Sources` section (not
    merely be the first `Ref` table anywhere in the template), and its cells
    must be exactly the six contract fields **in order** — duplicates,
    omissions, reordering, and extra columns are all contract breaks."""
