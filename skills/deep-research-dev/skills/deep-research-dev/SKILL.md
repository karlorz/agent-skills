...
0. **Status header** (precedes TL;DR) -- emit `**Status: Verified**` at the top of the report if every retained claim carries non-empty external verification (see §5 Freshness & Verification Status) and no coverage gaps remain; emit `**Status: Partial**` otherwise, listing the gaps inline. Status is **Partial** if any of: a question returned no usable output; a claim was dropped (malformed, over-cap, or lacking external verification); an uncertainty was reported; synthesis produced an invalid or empty body; a source-plan channel failed without a substitute. The status line must name only categories actually present in Coverage and uncertainty — do not hand-count claims or state a specific number when Coverage records a different count.
