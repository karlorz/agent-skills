...
# Originally from hermes-remote-backup skill (archived)
# ============================================================================
# Detects Caddy configs, systemd services, databases, credstore, Hermes state
# on a remote Linux host. Outputs JSON to stdout.
#
# Usage:
#   bash discover-hermes.sh <host> [--verbose]
#
# Example:
#   bash discover-hermes.sh sg01
#   bash discover-hermes.sh sg01 --verbose
#   bash discover-hermes.sh sg02-agent     # via ~/.ssh/config alias
# ============================================================================
...
if [ -z "$HOST" ]; then
  echo '{"error":"Usage: discover-hermes.sh <host>","status":"error"}' >&2
  exit 1
fi
