...
run_simplify_skill_contract_checks() {
  ...
  assert_contains "simplify skill independent authorship" "$skill" 'independently authored'
...
run_dev_loop_prep_prompt_contract_checks() {
  ...
  assert_contains "setup separates merge authority from CI" "$setup" '**Section F2 — Isolation and merge landing.**'
  assert_contains "setup proposes isolation recommended" "$setup" 'Isolate (recommended)'
  assert_contains "setup proposes landing recommended" "$setup" 'PR from feature branch (recommended)'
  assert_contains "setup emits allow local merge false default" "$setup" 'allow_local_merge: false'
  assert_contains "setup emits worktree policy enabled default" "$setup" 'worktree_policy:
  enabled: true'
  assert_contains "setup offers attended local merge menu" "$setup" 'Attended local-merge menu'
  assert_contains "setup distinguishes auto merge and local merge" "$setup" '`auto_merge` is GitHub and `allow_local_merge` is local finishing'
  assert_not_contains "setup no direct push recommended phrase" "$setup" 'direct push on the release branch, PR on feature branches'
